...
class ConnectorMonitor
    ...
    def self.create_model(data)
        ...
        model.connector_id = data.delete('id').to_i
        data.delete('auths')
        data.each do |k,v|
    ...
    def synchronize_with(data)
        @connector_id = data.delete 'id'
        @auth_users = data.delete('auths') if data.has_key?('auths')
        reconnect = false
    ...
    def event_connect(connection_string, tunnel_string)
        ...
        #@connector_queue.async do
            ...
            Logger.debug "\"#{tunnel_string}\" \"#{connection_string}\" #{@connector_id}"
              @task.setArguments(NSArray.arrayWithObjects("-2", connection_string, "-o SendEnv=CID", "-o SendEnv=TOKEN", "-R #{tunnel_string}:#{@host}:#{@port}", "-i", App.private_key_path, nil))
            po = NSPipe.new
    ...
    def setAuthUsers
        ...
        @set_auth_user_queue.async do
            data = {
                ...
                'auths' => { 'auths' => @auth_users }.to_json,
                'publish' => 'false'
            }
