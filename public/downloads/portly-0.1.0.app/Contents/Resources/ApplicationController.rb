...
class ApplicationController
    ...
    def startApp
        ...
        self.preferences_menu.setEnabled(true) if App.global.token
        loadConnectors
    end

    def getMACAddress
        pointer = Pointer.new('I')
        kernResult = MACAddress.findEthernetInterfaces(pointer)
        mac_address = MACAddress.getMACAddress(pointer[0])
        #Logger.debug mac_address.inspect
        App.global.mac_address = "#{mac_address[0]}:#{mac_address[1]}:#{mac_address[2]}:#{mac_address[3]}:#{mac_address[4]}:#{mac_address[5]}"
    end
    ...
    def loadConnectors
        @settings_path = NSBundle.mainBundle.pathForResource('Settings', ofType:'plist')
    ...
    def signOut
        self.preferences_menu.setEnabled(false)
        Dispatch::Queue.main.async do
            ...
            App.save!
            App.global.connectors.dup.each do |connector|
                Logger.debug connector.subdomain
                connector.disconnect
                connector.destroy_model
            end
        end
        File.delete(App.private_key_path) rescue nil
